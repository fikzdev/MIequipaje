...
<View style={{
  ...
  {lockers.filter(l => l.tipo !== 'grande').map((locker, idx) => {
    let bgColor = '#aaa';
    if (locker.estado === 'disponible') bgColor = '#4CAF50';
    else if (locker.estado === 'reservado') bgColor = '#FFA500';
    else if (locker.estado === 'ocupado') bgColor = '#E53935';
    ...
    const LockerBox = (
      <View
        style={{
          ...
          alignItems: 'center',
          position: 'relative',
          opacity: locker.estado === 'disponible' ? 1 : 0.7,
        }}
      >
        {/* Manilla */}
        <View style={{
    ...
    // Siempre retorna un componente
    return locker.estado === 'disponible' ? (
      <TouchableOpacity
        key={locker.id}
        onPress={() => {
          alert(`¿Reservar locker pequeño ${idx + 1}?`);
        }}
        activeOpacity={0.7}
      >
        {LockerBox}
      ...
      <View key={locker.id}>{LockerBox}</View>
    );
  })}
</View>

{/* Lockers grandes abajo */}
<View style={{
  width: '100%',
  flexDirection: 'row',
  flexWrap: 'wrap',
  ...
  {lockers.filter(l => l.tipo === 'grande').map((locker, idx) => {
    ...
    return locker.estado === 'disponible' ? (
      <TouchableOpacity
        key={locker.id}
        onPress={() => {
          alert(`¿Reservar locker grande ${idx + 1}?`);
        }}
        activeOpacity={0.7}
      >
        {LockerBox}
